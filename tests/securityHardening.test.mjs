...
test("Electron denies unexpected navigation, windows, permissions, and IPC senders", () => {
  ...
  assert.equal((mainSource.match(/requireTrustedAppSender\(event\)/g) ?? []).length, 15);
  assert.match(mainSource, /ipcMain\.handle\("desktop:set-locale"[\s\S]*?requireTrustedAppSender\(event\)/);
